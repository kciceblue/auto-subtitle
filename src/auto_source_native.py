"""Complete original-owner automatic-language acquisition; no semantic filtering."""
...
from src import contextual_asr_auto_native as auto
...
from src.contextual_asr_native import (_apply_policy,_same,_sha,_CAPTURE_KEYS,tensor_record,_TENSOR_KEYS,_config,
    _array,_now,_deadline,WorkerDeadline,_sealed,_load_tensor,_describe,_GEN_KEYS,_time,tensor_identity,_file,_read,_save)
from src.contextual_asr_auto_native import native_parse
VERSION='auto-source-worker-1'
REQUEST_VERSION='auto-source-request-1'
PLAN_VERSION='auto-source-plan-1'
OBSERVATIONS_VERSION='auto-source-observations-1'
POLICY={'language':None,'maximum_calls':66,'model_loads':1,'retries':0,'local_seconds':1200,
        'work_seconds':840,'cleanup_reserve_seconds':360,'control_calls':0,'recap_calls':0,
...
def validate_assets(assets,identity,*,runtime_check=False):
    require(_same(identity,execution_identity(assets)),'Automatic source worker identity changed')
...
def _stamp(text):
    ...
    require(match is not None,'Invalid immutable timestamp')
    h,m,s,ms=map(int,match.groups());require(m<60 and s<60,'Invalid timestamp component')
    return ((h*60+m)*60+s)*16000+ms*16


def owner_geometry(rows,detector):
    require(type(rows) is list and len(rows)==66,'Exactly66 original source owners required')
    frames=detector['mono']['frames'];require(type(frames) is int and frames>0 and detector['mono']['sample_rate']==16000,'Invalid mono geometry')
    speech=detector['vad_speech'];previous=0
    require(type(speech) is list,'VAD intervals missing')
    for segment in speech:
        require(type(segment) is dict and set(segment)=={'start','end'} and all(type(v) is int for v in segment.values())
            and previous<=segment['start']<segment['end']<=frames,'Invalid saved VAD interval')
        previous=segment['end']
    result=[];previous=0
    for number,row in enumerate(rows,1):
        require(set(row)=={'index','ts_line','text'} and type(row['index']) is int and row['index']==number,'Original owner identity changed')
        require(type(row['text']) is str and type(row['ts_line']) is str,'Original source row type changed')
        parts=row['ts_line'].split(' --> ');require(len(parts)==2,'Invalid timestamp range')
        start,end=map(_stamp,parts)
        require(start==previous and 0<end-start<=30*16000 and start<frames,'Owners must cover the continuous recording')
        physical_end=min(end,frames);clamped=end-physical_end
        require(clamped==0 or number==66 and clamped<=2,'Unexpected out-of-domain owner geometry')
        detected=sum(max(0,min(physical_end,s['end'])-max(start,s['start'])) for s in speech)
        result.append({'owner_id':number,'owner_ts_line':row['ts_line'],'owner_start_frame':start,'owner_end_frame':end,
            'crop_start_frame':start,'crop_end_frame':physical_end,'sample_rate':16000,'clamped_tail_frames':clamped,
            'vad':{'detected_frames':detected,'measured_frames':physical_end-start,'crop_frames':physical_end-start,
                   'owner_out_of_domain_frames':clamped}})
        previous=end
    require(result[-1]['crop_end_frame']==frames,'Original owners do not cover complete mono')
    return result


def build_plan(rows,detector,mono_sha256,audio_bindings,assets,control_proof_sha256):
    geometry=owner_geometry(rows,detector)
    require(type(audio_bindings) is dict and set(audio_bindings)=={str(i) for i in range(1,67)},'Complete66 audio bindings required')
    for row in geometry:
        a=audio_bindings[str(row['owner_id'])];ec._keys(a,{'path','sha256','pcm_sha256','frames','sample_rate','dtype'})
        require(type(a['frames']) is int and a['frames']==row['crop_end_frame']-row['crop_start_frame']
            and a['sample_rate']==16000 and a['dtype']=='float32' and Path(a['path']).is_absolute(),'Audio binding scope changed')
        for key in ('sha256','pcm_sha256'):require(type(a[key]) is str and re.fullmatch('[0-9a-f]{64}',a[key]),'Invalid audio digest')
    for digest in (mono_sha256,control_proof_sha256):require(type(digest) is str and re.fullmatch('[0-9a-f]{64}',digest),'Invalid input digest')
    value={'version':PLAN_VERSION,'source_rows_sha256':ec._hash(rows),'original_mono_sha256':mono_sha256,
        'original_mono_frames':detector['mono']['frames'],'sample_rate':16000,'geometry':geometry,
        'detector_record_sha256':ec._hash(detector),'audio_bindings':deepcopy(audio_bindings),
        'execution_identity':execution_identity(assets),'control_proof_sha256':control_proof_sha256,
        'capture_worker_sha256':_file(auto.__file__),'base_worker_sha256':_file(base.__file__),
        'policy':deepcopy(POLICY),'slots':[{'slot_id':f'owner-{i:03d}','owner_id':i} for i in range(1,67)]}
...
def check_plan(plan,assets):
    ec._keys(plan,{'version','source_rows_sha256','original_mono_sha256','original_mono_frames','sample_rate',
        'geometry','detector_record_sha256','audio_bindings','execution_identity','control_proof_sha256',
        'capture_worker_sha256','base_worker_sha256','policy','slots','plan_sha256'})
    require(plan['version']==PLAN_VERSION and plan['plan_sha256']==ec._hash({k:v for k,v in plan.items() if k!='plan_sha256'})
        and _same(plan['policy'],POLICY) and plan['capture_worker_sha256']==_file(auto.__file__)
        and plan['base_worker_sha256']==_file(base.__file__) and _same(plan['execution_identity'],execution_identity(assets))
        and _same(plan['slots'],[{'slot_id':f'owner-{i:03d}','owner_id':i} for i in range(1,67)])
        and len(plan['geometry'])==66,'Full automatic source plan changed')
    # Independent source/PCM reconstruction is performed by the controller.
    for i,row in enumerate(plan['geometry'],1):
        require(row['owner_id']==i and 0<=row['crop_start_frame']<row['crop_end_frame']<=plan['original_mono_frames']
            ...
            'Native geometry/binding mismatch')


def build_request(plan,owner_id):
    require(type(owner_id) is int and 1<=owner_id<=66,'Unknown original owner')
    return {'version':REQUEST_VERSION,'plan_sha256':plan['plan_sha256'],'owner_id':owner_id,
        'observation_id':f'auto-source-owner-{owner_id:03d}','view':deepcopy(plan['geometry'][owner_id-1]),
        'audio':deepcopy(plan['audio_bindings'][str(owner_id)]),'execution_identity':deepcopy(plan['execution_identity']),
        'policy':deepcopy(base.requests.POLICY),'language':None,'conditioning':deepcopy(CONDITIONING)}


def validate_request(request,plan):
    require(_same(request,build_request(plan,request['owner_id'])),'Automatic source request changed')


def _pcm(audio):
    import numpy as np
    require(isinstance(audio,np.ndarray) and audio.dtype==np.dtype('float32') and audio.ndim==1
        and 0<len(audio)<=30*16000 and np.isfinite(audio).all() and float(np.max(np.abs(audio)))<=1.,'Invalid variable-duration PCM')
...
def _read_audio(binding):
    ...
    path=Path(binding['path']);require(_file(path)==binding['sha256'],'Prepared owner WAV changed')
    info=sf.info(path)
    require(info.channels==1 and info.frames==binding['frames'] and info.samplerate==16000 and info.subtype=='FLOAT','Owner WAV geometry changed')
    audio,rate=sf.read(path,dtype='float32',always_2d=False);audio=_pcm(audio)
    require(rate==16000 and _sha(audio.tobytes())==binding['pcm_sha256'] and _file(path)==binding['sha256'],'Owner PCM binding changed')
    return np.array(audio,copy=True)


def protocol_category(raw,text):
    if not raw.strip():return 'empty_raw'
    if re.fullmatch(r'language None\s*<asr_text>\s*',raw.strip(),flags=re.IGNORECASE):return 'native_none_empty_tail'
    return 'nonempty_transcript' if text else 'unrecognized_empty_protocol'


def summarize(plan,outputs):
    require(set(outputs)=={s['slot_id'] for s in plan['slots']},'Complete66 native observations required')
    return {'available_slots':66,'unavailable_slots':0,'empty_slots':sum(v['parsed_text']=='' for v in outputs.values()),
        'detected_language_counts':dict(sorted(Counter(v['native_language'] for v in outputs.values()).items())),
...
def validate_native_receipt(receipt, request, plan, *, assets, artifact_root, processor):
    ...
    return {'raw_text': c['raw_text'], 'parsed_text': text, 'native_language': language,
...
def run_slots(plan,assets,folder,asr,audios,*,deadline,save_state):
    ...
    summary={'version':VERSION,'plan_sha256':plan['plan_sha256'],'status':'running','slots':slots,
        'asr_calls':0,'error_type':None,'available_slots':0,'unavailable_slots':66}
    ...
    for slot in slots:
        ...
        try:
            with _deadline(call_deadline):
                ...
                summary['available_slots']=len(outputs);summary['unavailable_slots']=66-len(outputs)
...
def run_worker(spec_path):
    ...
    require(spec['version']==VERSION and type(spec['worker_seconds']) in (int,float)
        and math.isfinite(spec['worker_seconds']) and 0<spec['worker_seconds']<=840,'Invalid real-pair worker budget')
    ...
    state={'version':VERSION,'status':'prepared','started_utc':_now(),'finished_utc':None,'seconds':0.,'asr_calls':0,
        'model_loads':0,'model_load_attempts':0,'error_type':None,'spec_sha256':_file(spec_path),'available_slots':0,'unavailable_slots':66}
    ...
    try:
        with _deadline(deadline):
            ...
            audios={str(i):_read_audio(plan['audio_bindings'][str(i)]) for i in range(1,67)}

"""Read-only W1 native replay, including source-only T1 ancestry and scalar gates.
...
from src import whole_owner_revisit as controller
...
def _source_ancestry(folder, conf, source, target, pools, track):
    ...
    for key in ('evidence_paths', 'pool_version', 'baseline_review', 'media', 'media_sha256',
        ...
        require(_same(conf[key], inherited[key]), 'W1 changed inherited T1 inputs')
...
def _writer_preflight(out, writer_requests, track):
    ...
    require(isinstance(value, dict) and set(value) == keys
        and value['version'] == 'whole-owner-writer-capacity-preflight-1'
        and value['round'] == 'W1' and value['family'] == 'gemma' and value['status'] == 'complete'
...
def _candidate(folder, conf, source, target, pack, bound, track):
    ...
    require(bool(accepted), 'No accepted non-noop whole-owner candidate')
    ...
    if rolled:
        ...
        require(bool(accepted), 'All whole-owner changes rolled back; no distinct candidate')
        final_source, final_target, ledger = whole.apply_owner_transactions(source, target, accepted, pack)
    ...
    require(set(checks) == set(workflow.rowmap(source)) and candidate['coverage'] == 66
        ...
        'Complete native whole-owner local gate did not pass')
    ...
    return final_target, {'owners_considered': 66, 'writer_groups': 33, 'writer_preflight_groups': len(preflight),
...
def _below_target_score(folder, conf, source, target, pools, baseline, track):
    ...
    def replay_score(path, *, candidate):
        ...
        require(score.get('status') == 'complete' and score.get('pool_version') == conf['pool_version']
            and score.get('target_sha256') == target_hash, 'Scalar does not bind whole-owner target')
...
def validate_campaign(folder: str | Path, *, require_four: bool = True) -> dict:
    """Replay W1 from immutable native records, with no inference or promotion."""
    ...
    if require_four:
        require(conf['status'] == 'confirmed_four' and conf.get('eligible_candidate') == controller.ROUND,
                'W1 has not reached a confirmed four')
    pins = {}
    def track(path, raw=False):
        ...
        require(str(path) not in pins or pins[str(path)] == digest, 'Artifact changed during whole-owner replay')
    ...
    require(required <= set(conf['code_pins']), 'Missing whole-owner producer snapshots')
    for path, digest in {**conf['input_hashes'], **conf['code_pins']}.items():
        track(path, raw=True); require(file_hash(Path(path)) == digest, 'Pinned whole-owner input or producer changed')
    ...
    require(len(source) == len(target) == 66
        ...
        'Original whole-owner geometry changed')
    ...
    return {'version': controller.VERSION, 'round': controller.ROUND,
        ...
        'local': local, 'fallback_owner_ids': fallback_ids, 'pack_sha256': pack['pack_sha256'],

"""One capacity-only, losslessly encoded whole-owner successor; all writing local."""
...
from src import compact_owner_requests as wr
...
PLAN=ROOT/'design/quality-compact-owner-20260915.md'
CAPACITY_FAILURE=ROOT/'output/quality-whole-owner-20260915'
VERSION='compact-owner-local-revisit-1'
ROUND='W2'
...
def init(folder):
    ...
    failed=previous.check(CAPACITY_FAILURE)
    failed_preflight=read(CAPACITY_FAILURE/'W1/writer-capacity-preflight.json')
    require(failed['status']=='failed' and failed.get('error_type')=='RuntimeError'
        ...
        and failed_preflight.get('status')=='failed' and failed_preflight.get('round')=='W1',
        'W2 requires the preserved restored W1 capacity failure')
    ...
    require(not list((CAPACITY_FAILURE/'W1/requests').glob('*.json'))
        and not (CAPACITY_FAILURE/'W1/requests/metrics.jsonl').exists()
        and not any((CAPACITY_FAILURE/'W1'/name).exists() for name in
            ('candidate.json','proposed-transactions.json','target.json','score.json')),
        'W1 already generated or scored a candidate; not a capacity-only successor')
    ...
    require(baseline['purpose']=='baseline' and baseline['pool_version']==old['pool_version']
        ...
        and baseline['inputs']['context']['sha256']==old['context_sha256'],'W2 baseline drift')
    folder.mkdir(parents=True,exist_ok=True);require(not any(folder.iterdir()),'Unregistered W2 output exists')
    pins={str(PLAN):file_hash(PLAN),str(PREDECESSOR/'campaign.json'):file_hash(PREDECESSOR/'campaign.json')}
    for path in (CAPACITY_FAILURE/'campaign.json',CAPACITY_FAILURE/'W1/writer-capacity-preflight.json'):
    ...
    conf.update(version=VERSION,status='prepared',created_utc=sr.now(),target=4,maximum_candidates=1,
        ...
        input_hashes=pins,predecessor=str(PREDECESSOR),capacity_failure_predecessor=str(CAPACITY_FAILURE),prior_score_paths=prior,external_feedback='score_only',
...
def check(folder):
    ...
    require(conf['version']==VERSION and conf['maximum_candidates']==1 and conf['maximum_local_seconds']==LIMIT
        ...
        ('source_edits_allowed','raw_source_fidelity_verified','human_reference_used','visual_input_used','new_audio_acquired','selected_output_modified','writer_sees_old_chinese')),'W2 scope drift')
    failure=Path(conf['capacity_failure_predecessor'])
    require(failure.is_absolute() and all(str(failure/name) in conf['input_hashes']
        for name in ('campaign.json','W1/writer-capacity-preflight.json')),
        'Capacity failure predecessor bindings missing')
    for name,digest in {**conf['input_hashes'],**conf.get('code_pins',{})}.items():require(file_hash(Path(name))==digest,'Pinned W2 input changed')
    return conf


def freeze(folder):
    conf=check(folder)
    names=['compact_owner_revisit','compact_owner_release','compact_owner_requests','whole_owner_revisit','whole_owner_release','whole_owner','whole_owner_requests','temporal_revisit','temporal_release','temporal_evidence','temporal_source_frames','short_audio','recap_index',
...
def validate_candidate_artifacts(folder):
    ...
    require(candidate['version']==VERSION and candidate['round']==ROUND and candidate['source_edits_allowed'] is False
        and candidate['writer_sees_old_chinese'] is False,'W2 candidate scope changed')
    ...
    require(candidate['pack_sha256']==conf['source_pack_sha256']==pack['pack_sha256']
        and candidate['map_sha256']==conf['map_sha256'],'W2 candidate evidence changed')
    ...
    require(ledger==candidate['transaction_ledger'] and result_source==source
        ...
        and file_hash(out/'source.json')==candidate['source_sha256'] and file_hash(out/'target.json')==candidate['target_sha256'], 'W2 candidate artifact changed')
    require(candidate['changed_target_owners']==[a.index for a,b in zip(result_target,target) if a.text!=b.text],'W2 change ledger drift')
...
def score(folder,baseline):
    ...
    require(bundle['pool_version']==baseline['pool_version'] and bundle['inputs']['target']['sha256']==target_hash
        and bundle['inputs']['source']['sha256']==er.owner_rows_sha256(source),'W2 score bundle drift')
    ...
    result['status']='complete';write(result_path,result);LOG.info('W2 independent score=%s confirmed=%s',result['score'],result['confirmed']);return result
...
def execute(folder):
    ...
    try:
        ...
        if check(folder)['local_seconds']>=LIMIT:raise rw.LocalBudgetExceeded('W2 budget exhausted before scoring')
        ...
        try:_restore(folder,previous)
            ...
            primary_error.add_note('W2 restoration failed: '+type(exc).__name__)
    return check(folder)


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--campaign',type=Path,default=ROOT/'output/quality-compact-owner-20260915')

"""Read-only L1 native replay, including source-only T1 ancestry and scalar gates.
...
from src import episode_draft_native as writer_native
from src import whole_owner_requests as original_requests
from src import compact_owner_revisit as compact_controller
from src import compact_owner_release as compact_gate
from src import episode_draft_requests as requests
...
def _source_ancestry(folder, conf, source, target, pools, track):
    ...
    for key in ('evidence_paths', 'pool_version', 'baseline_review', 'media', 'media_sha256',
        ...
        require(_same(conf[key], inherited[key]), 'L1 changed inherited T1 inputs')
...
def _compact_capacity_failure_ancestry(folder, conf, source, target, pools, pack, bound, track):
    """Replay the immutable W2 failure; it must have made no generation calls."""
    name = conf.get('compact_capacity_failure_predecessor')
    require(isinstance(name, str) and Path(name).is_absolute(), 'W2 capacity-failure predecessor is missing')
    parent = Path(name).resolve(strict=True)
    conf_path = parent/'campaign.json'; preflight_path = parent/'W2/writer-capacity-preflight.json'
    for path in (conf_path, preflight_path):
        require(conf['input_hashes'].get(str(path)) == file_hash(path),
                'W2 capacity-failure prerequisite was not input-pinned')
    prior = compact_controller.check(parent)
    require(_same(track(conf_path), prior) and prior.get('version') == compact_controller.VERSION
        ...
        'W2 must be a restored preflight failure without an eligible candidate')
    for key in ('source_sha256', 'target_sha256', 'context_sha256', 'evidence_paths', 'pool_version',
                ...
                'capacity_failure_predecessor'):
        require(_same(conf[key], prior[key]), 'W2 changed the failed W2 comparison inputs')
    old._producer_snapshots(parent, prior, track)
    required = {str(compact_controller.ROOT/'src'/f'{name}.py') for name in
    ...
    require(required <= set(prior['code_pins']), 'Failed W2 producer snapshots are incomplete')
    for path, digest in {**prior['input_hashes'], **prior['code_pins']}.items():
        track(path, raw=True)
        require(file_hash(Path(path)) == digest, 'Failed W2 producer or input changed')
    prior_pack, prior_bound, _, _ = compact_gate._source_ancestry(parent, prior, source, target, pools, track)
    require(_same(pack, prior_pack) and _same(bound, prior_bound), 'W2 changed W2 source-only frames or recap')
    out = parent/'W2'; request_dir = out/'requests'
    require(not any(request_dir.glob('*.json')) and not (request_dir/'metrics.jsonl').exists(),
            'W2 generated native requests before its failed preflight')
    require(not any((out/name).exists() for name in ('candidate.json', 'target.json', 'source.json',
        ...
        'Failed W2 contains generated candidate artifacts')
    ...
    original_requests = compact_controller.writer_requests(pack, bound)
    compact_gate._lossless_writer_requests(pack, bound, original_requests)
    require(isinstance(value, dict) and set(value) == keys
        and value['version'] == compact_controller.WRITER_PREFLIGHT_VERSION
        and value['round'] == 'W2' and value['family'] == 'gemma' and value['status'] == 'failed'
        ...
        and value['preflight_sha256'] == fingerprint({key: item for key, item in value.items()
            if key != 'preflight_sha256'}), 'Failed W2 preflight envelope changed')
    records = value['records']
    require(isinstance(records, list) and 1 <= len(records) <= 33,
            'Failed W2 preflight must preserve its attempted prefix')
    started = compact_gate._timestamp(value['started_utc'], 'W2 preflight start')
    finished = compact_gate._timestamp(value['finished_utc'], 'W2 preflight finish')
    ...
    for ordinal, (record, request) in enumerate(zip(records, original_requests), 1):
        require(isinstance(record, dict) and set(record) == record_keys
            ...
            and record['native_request_sha256'] == fingerprint(compact_controller.writer_native_request(request))
            and all(record[name+'_sha256'] == fingerprint(request[name]) for name in ('instruction', 'body', 'schema')),
            'Failed W2 preflight request prefix changed')
        start = compact_gate._timestamp(record['started_utc'], 'W2 group start')
        end = compact_gate._timestamp(record['finished_utc'], 'W2 group finish')
        require(previous_finish <= start <= end <= finished
            ...
            'Failed W2 preflight group timing changed')
        previous_finish = end
        if ordinal < len(records):
            require(record['error_type'] is None, 'W2 preflight retried after an earlier error')
            compact_gate._capacity(record['capacity'])
        else:
            require(record['error_type'] == 'RuntimeError' and record['capacity'] is None,
                    'W2 final preflight record does not preserve its native capacity failure')
...
def _global(out, source, before, after, transactions, pack, bound, prefix, native):
    ...
    for number, ids in enumerate(groups, 1):
        ...
        require(_same(request, original_requests.build_global_request(pack, bound, source, before, after, transactions, ids)),
                'L1 global verifier differs from unchanged whole-owner verification')
...
def _candidate(folder, conf, source, target, pack, bound, track):
    ...
    request = requests.build_writer_request(pack, bound)
    expected_body = {'source_evidence': temporal.prepare_recap_request(pack)['body'],
        'source_map': bound, 'focus_owner_ids': list(range(1, 67))}
    require(_same(request['body'], expected_body), 'Joint writer source projection differs from exact episode evidence and map')
    require(set(request['schema']['properties']['owners']['properties']) == {str(i) for i in range(1, 67)},
            'Joint writer output does not cover all 66 owners')
    ...
    if conf.get('native_attempt_recovery'):
        ...
        value = episode_draft_recovery.replay_native(folder, request, track)
    else:
    ...
    for transaction in proposed:
        ...
        require(_same(request, original_requests.build_verifier_request(pack, bound, source, target, transaction)),
                'L1 owner verifier differs from unchanged whole-owner verification')
    ...
    require(bool(accepted), 'No accepted non-noop episode-draft candidate')
    ...
    if rolled:
        ...
        require(bool(accepted), 'All episode-draft changes rolled back; no distinct candidate')
        final_source, final_target, ledger = whole.apply_owner_transactions(source, target, accepted, pack)
    ...
    require(set(checks) == set(workflow.rowmap(source)) and candidate['coverage'] == 66
        ...
        'Complete native episode-draft local gate did not pass')
...
def _below_target_score(folder, conf, source, target, pools, baseline, track):
    ...
    def replay_score(path, *, candidate):
        ...
        require(score.get('status') == 'complete' and score.get('pool_version') == conf['pool_version']
            and score.get('target_sha256') == target_hash, 'Scalar does not bind episode-draft target')
...
def validate_campaign(folder: str | Path, *, require_four: bool = True) -> dict:
    """Replay L1 from immutable native records, with no inference or promotion."""
    ...
    if require_four:
        require(conf['status'] == 'confirmed_four' and conf.get('eligible_candidate') == controller.ROUND,
                'L1 has not reached a confirmed four')
    pins = {}
    def track(path, raw=False):
        ...
        require(str(path) not in pins or pins[str(path)] == digest, 'Artifact changed during episode-draft replay')
    ...
    if conf.get('native_attempt_recovery'):
        from src import episode_draft_recovery
        episode_draft_recovery.validate_registration(folder, conf, track)
    ...
    required = {str(controller.ROOT/'src'/f'{name}.py') for name in
        ('episode_draft_revisit', 'episode_draft_release', 'episode_draft_requests', 'episode_draft_native',
            ...
            'whole_owner_revisit', 'whole_owner_release', 'whole_owner', 'whole_owner_requests')}
    if conf.get('native_attempt_recovery'):
        required.add(str(controller.ROOT/'src/episode_draft_recovery.py'))
    require(required <= set(conf['code_pins']), 'Missing episode-draft producer snapshots')
    for path, digest in {**conf['input_hashes'], **conf['code_pins']}.items():
        track(path, raw=True); require(file_hash(Path(path)) == digest, 'Pinned episode-draft input or producer changed')
    ...
    require(len(source) == len(target) == 66
        ...
        'Original episode-draft geometry changed')
    ...
    failures = {
        'W1': compact_gate._capacity_failure_ancestry(folder, conf, source, target, pools, pack, bound, track),
        'W2': _compact_capacity_failure_ancestry(folder, conf, source, target, pools, pack, bound, track)}
    ...
    return {'version': controller.VERSION, 'round': controller.ROUND,
        ...
        'writer_generation_reused': bool(conf.get('native_attempt_recovery')), 'fallback_owner_ids': fallback_ids, 'pack_sha256': pack['pack_sha256'],

"""R1 native writer with exact readable request binding and frozen L1 mechanics.

The controller owns validated projection/input/producer pins and the local
...
from src import readable_draft_requests as drafts
...
VERSION = 'readable-draft-native-1'
...
def prepare_request(request: dict) -> dict:
    """Independent R1 identity with the exact frozen Qwen settings and reserve."""
    require(isinstance(request, dict) and set(request) == {'instruction', 'body', 'schema'},
            'Invalid readable writer request')
    require(request['instruction'] == drafts.WRITER_INSTRUCTION and _same(request['schema'], drafts.writer_schema()),
            'Readable instruction or all-owner output schema changed')
    body = request['body']
    require(isinstance(body, dict) and set(body) == {'source_evidence', 'focus_owner_ids'}
            and _same(body['focus_owner_ids'], list(drafts.OWNER_IDS)), 'Readable writer input scope changed')
    evidence = body['source_evidence']
    require(isinstance(evidence, dict) and set(evidence) == readable._PROMPT_KEYS
            and isinstance(evidence['views'], list) and bool(evidence['views'])
            and isinstance(evidence['source_rows'], list) and len(evidence['source_rows']) == 66
            and all(isinstance(row, dict) for row in evidence['source_rows'])
            and _same([row.get('index') for row in evidence['source_rows'] if isinstance(row, dict)],
                      list(drafts.OWNER_IDS)), 'Complete grouped readable source evidence missing')
    # The source-bound builder and release independently reconstruct every field.
...
def config_for(endpoint: str, directory: Path, key: str, request: dict, remaining: float = 600) -> TranslateConfig:
    ...
    require(type(remaining) in (int, float) and math.isfinite(remaining), 'Invalid readable deadline')
    if remaining < 1:
        raise rw.LocalBudgetExceeded('No remaining readable draft budget')
...
def replay_native(path: Path, request: dict, track: Track) -> dict:
    ...
    require(isinstance(saved, dict) and set(saved) == _ROOT_KEYS
            ...
            'Readable native request or sealed ledger changed')
    attempts = saved['attempts']
    require(saved['status'] == 'complete' and isinstance(attempts, list) and 1 <= len(attempts) <= MAX_ATTEMPTS,
            'Readable request lacks bounded complete execution')
    ...
    require(used_lines == [index for index, metric in enumerate(metrics) if metric.get('stage') == path.stem],
            'Unregistered native readable invocation or hidden retry')
    require(_same(parsed, saved['parsed']), 'Parsed readable draft differs from raw complete answer')
...
def ask(directory: Path, key: str, endpoint: str, request: dict) -> dict:
    ...
    if path.exists():
        ...
        require(saved.get('status') == 'complete', 'Incomplete saved readable request cannot be resumed')
    ...
    for number in range(1, MAX_ATTEMPTS + 1):
        ...
        if number == MAX_ATTEMPTS:
            raise RuntimeError('One readable writer technical retry exhausted') from failure
    raise AssertionError('Unreachable readable draft retry state')

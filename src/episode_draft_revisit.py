"""One joint whole-episode Chinese draft on local Qwen with strict owner checks."""
...
from src import episode_draft_requests as wr
from src import episode_draft_native as native
from src import whole_owner_revisit as previous
from src import compact_owner_revisit as compact_previous
...
PLAN=ROOT/'design/quality-episode-draft-20260915.md'
CAPACITY_FAILURE=ROOT/'output/quality-whole-owner-20260915'
COMPACT_CAPACITY_FAILURE=ROOT/'output/quality-compact-owner-20260915'
VERSION='episode-draft-local-revisit-1'
ROUND='L1'
WRITER_REQUEST_KEY='episode-draft'
...
def init(folder):
    ...
    for failed_folder,failed_round,module in (
        (CAPACITY_FAILURE,'W1',previous),(COMPACT_CAPACITY_FAILURE,'W2',compact_previous)):
        failed=module.check(failed_folder)
        failed_preflight=read(failed_folder/failed_round/'writer-capacity-preflight.json')
        require(failed['status']=='failed' and failed.get('error_type')=='RuntimeError'
            and failed.get('original_backend_restored') is True and not failed.get('eligible_candidate')
            and failed_preflight.get('status')=='failed' and failed_preflight.get('round')==failed_round,
            'L1 requires preserved restored W capacity failures')
        require(all(failed[k]==old[k] for k in ('source_sha256','target_sha256','context_sha256','pool_version')),
            'Capacity predecessor inputs differ from T1')
        failed_out=failed_folder/failed_round
        require(not list((failed_out/'requests').glob('*.json'))
            and not (failed_out/'requests/metrics.jsonl').exists()
            and not any((failed_out/name).exists() for name in
                ('candidate.json','proposed-transactions.json','target.json','score.json')),
            'Closed capacity direction already generated or scored a candidate')
    ...
    require(baseline['purpose']=='baseline' and baseline['pool_version']==old['pool_version']
        ...
        and baseline['inputs']['context']['sha256']==old['context_sha256'],'L1 baseline drift')
    folder.mkdir(parents=True,exist_ok=True);require(not any(folder.iterdir()),'Unregistered L1 output exists')
    pins={str(PLAN):file_hash(PLAN),str(PREDECESSOR/'campaign.json'):file_hash(PREDECESSOR/'campaign.json')}
    for failed_folder,failed_round in ((CAPACITY_FAILURE,'W1'),(COMPACT_CAPACITY_FAILURE,'W2')):
        for path in (failed_folder/'campaign.json',failed_folder/failed_round/'writer-capacity-preflight.json'):
    ...
    conf.update(version=VERSION,status='prepared',created_utc=sr.now(),target=4,maximum_candidates=1,
        ...
        input_hashes=pins,predecessor=str(PREDECESSOR),capacity_failure_predecessor=str(CAPACITY_FAILURE),compact_capacity_failure_predecessor=str(COMPACT_CAPACITY_FAILURE),prior_score_paths=prior,external_feedback='score_only',
        source_edits_allowed=False,raw_source_fidelity_verified=False,human_reference_used=False,visual_input_used=False,
        new_audio_acquired=False,selected_output_modified=False,writer_sees_old_chinese=False,maximum_writer_groups=1,maximum_owner_checks=66)
...
def check(folder):
    ...
    require(conf['version']==VERSION and conf['maximum_candidates']==1 and conf['maximum_local_seconds']==LIMIT
        and conf['maximum_round_seconds']==LIMIT and conf['maximum_writer_groups']==1 and conf['maximum_owner_checks']==66
        and conf['external_feedback']=='score_only' and all(conf[k] is False for k in
        ('source_edits_allowed','raw_source_fidelity_verified','human_reference_used','visual_input_used','new_audio_acquired','selected_output_modified','writer_sees_old_chinese')),'L1 scope drift')
    for key,failed_round in (('capacity_failure_predecessor','W1'),('compact_capacity_failure_predecessor','W2')):
        failure=Path(conf[key])
        require(failure.is_absolute() and all(str(failure/name) in conf['input_hashes']
            for name in ('campaign.json',failed_round+'/writer-capacity-preflight.json')),
            'Closed capacity failure bindings missing')
    for name,digest in {**conf['input_hashes'],**conf.get('code_pins',{})}.items():require(file_hash(Path(name))==digest,'Pinned L1 input changed')
    if conf.get('native_attempt_recovery'):
        from src import episode_draft_recovery
        episode_draft_recovery.validate_registration(folder,conf)
    return conf
...
def freeze(folder):
    conf=check(folder)
    if conf.get('native_attempt_recovery'):
        manifest=read(Path(conf['producer_snapshot_manifest']))
        require(set(manifest)==set(conf['code_pins']), 'Recovery producer set differs')
        for original,spec in manifest.items():
            require(spec['sha256']==conf['code_pins'][original]==file_hash(Path(spec['snapshot'])), 'Recovery producer snapshot changed')
        return
    names=['episode_draft_revisit','episode_draft_release','episode_draft_requests','episode_draft_native','compact_owner_revisit','compact_owner_release','compact_owner_requests','whole_owner_revisit','whole_owner_release','whole_owner','whole_owner_requests','temporal_revisit','temporal_release','temporal_evidence','temporal_source_frames','short_audio','recap_index',
...
def writer_request(pack,bound):
    return wr.build_writer_request(pack,bound)
...
def build_candidate(folder,pack,bound):
    ...
    request=writer_request(pack,bound)
    if conf.get('native_attempt_recovery'):
        from src import episode_draft_recovery
        with measured(folder,ROUND,'saved_episode_draft_mechanical_replay'):
            raw=episode_draft_recovery.replay_native(folder,request,episode_draft_recovery._track)
    else:
        with measured(folder,ROUND,'source_only_joint_episode_generation'):
...
def validate_candidate_artifacts(folder):
    ...
    require(candidate['version']==VERSION and candidate['round']==ROUND and candidate['source_edits_allowed'] is False
        and candidate['writer_sees_old_chinese'] is False,'L1 candidate scope changed')
    ...
    require(candidate['pack_sha256']==conf['source_pack_sha256']==pack['pack_sha256']
        and candidate['map_sha256']==conf['map_sha256'],'L1 candidate evidence changed')
    ...
    require(ledger==candidate['transaction_ledger'] and result_source==source
        ...
        and file_hash(out/'source.json')==candidate['source_sha256'] and file_hash(out/'target.json')==candidate['target_sha256'], 'L1 candidate artifact changed')
    require(candidate['changed_target_owners']==[a.index for a,b in zip(result_target,target) if a.text!=b.text],'L1 change ledger drift')
...
def score(folder,baseline):
    ...
    require(bundle['pool_version']==baseline['pool_version'] and bundle['inputs']['target']['sha256']==target_hash
        and bundle['inputs']['source']['sha256']==er.owner_rows_sha256(source),'L1 score bundle drift')
    ...
    result['status']='complete';write(result_path,result);LOG.info('L1 independent score=%s confirmed=%s',result['score'],result['confirmed']);return result
...
def execute(folder):
    ...
    try:
        ...
        if check(folder)['local_seconds']>=LIMIT:raise rw.LocalBudgetExceeded('L1 budget exhausted before scoring')
        ...
        try:_restore(folder,previous)
            ...
            primary_error.add_note('L1 restoration failed: '+type(exc).__name__)
    return check(folder)


def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--campaign',type=Path,default=ROOT/'output/quality-episode-draft-20260915')

...
VERSION, ROUND = 'masked-source-screen-2', 'AM-G1'
FOLDER = ROOT/'output/quality-masked-source-draft-continuation-20260916'
FAILED_FOLDER = ROOT/'output/quality-masked-source-draft-20260916'
FAILURE_RECORD = ROOT/'design/am-g1-preparation-hash-failure-20260916/failure.json'
FAILED_PREPARATION_SECONDS = 0.3209794379945379
...
COPIES = ('source.json','original-context.txt','B-request.json','observations.json','acquisition-proof.json','request.json','native-request.json','continuation.json')
...
def acquisition_proof(track):
    ...
    require(same(result['attention_policy'],contract.ATTENTION_POLICY)
        and result['attention_source_sha256'] == ec._hash(plan['attention_source'])
        and result['source_rows_sha256'] == observation['source_rows_sha256']
        and result['geometry_sha256'] == ec._hash(plan['parent_plan']['geometry'])
        and result['observation_envelope_sha256'] == ec._hash(observation)
...
def continuation(track):
    record = track(FAILURE_RECORD); failed_path = (FAILED_FOLDER/'screen.json').resolve()
    failed = track(failed_path)
    require(record['status'] == 'pre_generation_failure' and record['model_calls'] == 0
        and record['native_request_registered'] is False and record['failed_state_path'] == str(failed_path)
        and record['failed_state_sha256'] == file_hash(failed_path)
        and same(record['failed_local_seconds'],FAILED_PREPARATION_SECONDS),'Saved pre-inference failure changed')
    require({p.name for p in FAILED_FOLDER.iterdir()} <= {'.lock','screen.json'}
        and failed['version'] == 'masked-source-screen-1' and failed['round'] == ROUND and failed['status'] == 'failed'
        and failed['error_type'] == 'ValueError' and failed['original_backend_restored'] is True
        and same(failed['local_seconds'],FAILED_PREPARATION_SECONDS) and same(failed['work_seconds'],FAILED_PREPARATION_SECONDS)
        and failed['cleanup_seconds'] == 0 and failed.get('score') is None
        and same(failed['stages'],[{'stage':'local_input_preparation','seconds':FAILED_PREPARATION_SECONDS,
            'cleanup':False,'error_type':'ValueError'}]),'Prior preparation was not the preserved zero-generation failure')
    require({Path(p).name for p in record['saved_files']} == {'masked_source_screen.py','masked_source_draft_requests.py','test_masked_source_screen.py'},
            'Failed producer snapshots missing')
    for name,digest in record['saved_files'].items():
        path = (ROOT/name).resolve(); require(path.parent == FAILURE_RECORD.parent.resolve(),'Failed snapshot path changed')
        track(path,raw=True); require(file_hash(path) == digest,'Failed producer snapshot changed')
    return {'version':'masked-source-preparation-continuation-1','failed_state':str(failed_path),
        'failed_state_sha256':record['failed_state_sha256'],'failure_record':str(FAILURE_RECORD.resolve()),
        'failure_record_sha256':file_hash(FAILURE_RECORD),'carried_local_seconds':FAILED_PREPARATION_SECONDS,
        'carried_work_seconds':FAILED_PREPARATION_SECONDS,'original_generation_calls':0,
        'native_request_registered':False,'repair':'Use producer ec._hash for native hashes; writer fingerprints unchanged'}
...
def validate_registration(folder):
    ...
    track = Tracker(); note = continuation(track); current = state(folder)
    carry = [entry for entry in current['stages'] if entry['stage'] == 'inherited_failed_preparation']
    require(same(note,read(folder/'continuation.json')) and reg['continuation_sha256'] == file_hash(folder/'continuation.json')
        and same(carry,[{'stage':'inherited_failed_preparation','seconds':FAILED_PREPARATION_SECONDS,'cleanup':False,'error_type':'ValueError'}])
        and current['local_seconds'] >= FAILED_PREPARATION_SECONDS and current['work_seconds'] >= FAILED_PREPARATION_SECONDS,
        'Continuation carry was changed or applied more than once')
...
def prepare(folder=FOLDER):
    ...
    with locked(folder):
        ...
        write_json(folder/'screen.json',{'version':VERSION,'round':ROUND,'status':'preparing','local_seconds':FAILED_PREPARATION_SECONDS,
            'work_seconds':FAILED_PREPARATION_SECONDS,'cleanup_seconds':0.,
            'stages':[{'stage':'inherited_failed_preparation','seconds':FAILED_PREPARATION_SECONDS,'cleanup':False,'error_type':'ValueError'}],'original_backend_restored':True,'qualified':False,'score':None})
        try:
            with counted(folder,'local_input_preparation'):
                track = Tracker(); note = continuation(track); immutable(folder/'continuation.json',note)
                b,source,context = inputs(track)
                observations,proof = acquisition_proof(track)
                ...
                reg = {'version':VERSION,'round':ROUND,'settings':asdict(SETTINGS),'limits':[LIMIT,WORK_LIMIT,CLEANUP_LIMIT],
                       ...
                       'continuation_sha256':file_hash(folder/'continuation.json')}
